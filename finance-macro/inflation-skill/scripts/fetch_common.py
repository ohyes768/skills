...
def published_month(target_month: str, today: datetime, publish_day: int) -> str:
    """判断目标月份数据是否已发布，未发布则降级到上月。

    数据在 next_month 的 publish_day 发布。
    例如：2026-03 月数据，在 2026-04-09 发布。
    """
    ...
    if next_month > 12:
        next_month = 1
        next_year += 1
    publish_dt = datetime(next_year, next_month, publish_day, tzinfo=timezone.utc)
    if today >= publish_dt:
        return target_month
