...
配置来源(优先 finance-macro/.env,其次 skill 根 .env):
    ...
    MACRO_UPLOAD_SSL_VERIFY=0   # NAS 自签证书场景跳过 TLS 校验

用法示例:
    uv run python scripts/upload_signal.py                # 推送 macro_signal.json(默认输入、默认 skill)
    uv run python scripts/upload_signal.py --dry-run      # 只做本地预检并打印 payload 摘要,不发送
    uv run python scripts/upload_signal.py --verify       # 推送并验证月份列表出现新月份
"""
...
LOGGER = logging.getLogger("upload_signal")
...
DEFAULT_SKILL = "monetary-policy-skill"
...
def load_env_file() -> None:
    ...
    candidates = [_SKILL_ROOT.parent / ".env", _SKILL_ROOT / ".env"]
...
def validate_payload(skill: str, file: str, data: Any) -> tuple[list[str], list[str]]:
    ...
    if SKILL_FILE_MAP[skill] != file:
        errors.append(f"skill 与 file 不匹配: {skill} 只能推送 {SKILL_FILE_MAP[skill]},收到 {file}")
        return errors, warns
...
def main() -> int:
    ...
    parser.add_argument("--input", type=str, default="",
                        help="输入 JSON 路径(默认: output/<skill>/<推断的file>)")
    parser.add_argument("--skill", type=str, default=DEFAULT_SKILL,
                        help=f"skill 名(默认 {DEFAULT_SKILL},白名单见 SKILL_FILE_MAP)")
    ...
    candidates = [
        _SKILL_ROOT.parent / "output" / _SKILL_ROOT.name / file,  # finance-macro/output/<skill>/<file>
        _SKILL_ROOT / file,                                       # <skill根>/<file>(旧位置)
    ]
    if args.input:
    ...
    if not input_path.exists():
        LOGGER.error("输入文件不存在: %s(macro_signal.json 请先运行 build_macro_signal.py)", input_path)
        return 1
