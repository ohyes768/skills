...
class GenerateMacroSignalsTests(unittest.TestCase):
    def test_generates_standard_signal_files_from_existing_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            out = root / "output"
            self._write_json(out / "monetary-policy-skill" / "2026-05" / "dr007.json", {"value": 1.6, "published_at": "2026-05-20"})
            self._write_json(out / "monetary-policy-skill" / "2026-05" / "lpr.json", {"lpr_1y": 3.0, "prev_lpr_1y": 3.1, "month": "2026-05"})
            self._write_json(out / "money-supply-skill" / "money_supply_latest.json", {
                "as_of_date": "2026-05-20",
                "m1_m2": {"latest": {"m2_yoy": 8.8, "m1_yoy": 5.8, "m1_m2_spread": -3.0}},
                "social_financing": {"monthly_new_yoy_pct": 5.0, "balance_yoy_pct": 8.2},
            })
            self._write_json(out / "entity-economy-skill" / "2026-05" / "electricity.json", {"month": "2026-05", "yoy_percent": 5.0})
            self._write_json(out / "entity-economy-skill" / "2026-05" / "railway_freight.json", {"month": "2026-05", "yoy_percent": 3.0})
            self._write_json(out / "inflation-skill" / "2026-05" / "inflation.json", {
                "month": "2026-05",
                "cpi": {"cpi_national_yoy": 1.0},
                "ppi": {"ppi_yoy": 0.5},
                "core_cpi": {"core_cpi_yoy": 1.1},
            })
            self._write_json(out / "exchange-rate-skill" / "exchange_rate_data.json", {
            ...
            for name in signals.DIMENSIONS:
                payload = json.loads((out / signals.FOLDERS[name] / "macro_signal.json").read_text(encoding="utf-8"))
